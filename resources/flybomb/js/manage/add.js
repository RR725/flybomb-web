...
let Add = React.createClass({
    ...
    getContent() {
        ...
        if (type == '1') {
            ...
            html = listRadio.map(function (data, key) {
                return (
                    <Row style={{ height: 44 }} key={key}>
                        <Col span="1">{data}</Col>
                        <Col span="23">
                            {getFieldDecorator('content_radio_' + data, {
                                initialValue: editData
                                    ? editData.content && editData.content[key]
                                    : '',
                                validate: [
                                    {
                                        rules: [
    ...
    getAnswer() {
        ...
        if (type == '1') {
            ...
            let radios = listRadio.map(function (data, key) {
                return (
                    <Col key={key} span="3">
                        <Radio style={radioStyle} value={data}>
                            {data}
                        </Radio>
                    </Col>
                )
    ...
    render() {
        ...
        let options = subjectList.map(function (data, key) {
            return (
                <Select.Option key={key} data={data} value={data.name}>
                    {data.name}
                </Select.Option>
            )
        })
        let typeOptions = questionType.map(function (data, key) {
            return (
                <Select.Option key={key} value={String(data.type)}>
                    {data.type_desc}
                </Select.Option>
            )
        })
            ...
            <div>
                <div className="home_toolbar">
                    <Row>
                        <Col span="20">
                            <div className="title">
                                <span className="border"></span>新建
                            </div>
                        </Col>
                        <Col span="4">
                            <Button onClick={this.add} type="primary" size="large">
                                新建科目
                            </Button>
                        </Col>
                    </Row>
                    ...
                    <Row className="add_app">
                        <Col span="18">
                            ...
                            <FormItem {...formItemLayout} label="类型">
                                ...
                                })(<Select style={{ width: 300 }}>{typeOptions}</Select>)}
                            </FormItem>
