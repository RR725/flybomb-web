...
const Question = React.createClass({
    ...
    componentWillReceiveProps() {
        ...
        if (type !== this.state.type && !this.state.status) {
            //用在点了相关推荐，且type值有变化，返回上一个页面
            this.randomDo()
        }
    },
    ...
    setErrQuestion(event, option, type) {
        ...
        if (type === 'add') {
            ...
            if (!option) return
            var request = transaction.objectStore(table).delete(option.questionId) // 通过键questionId来删除
            request.onsuccess = function (event) {
                console.log('删除成功！')
    ...
    render() {
        ...
        if (!question.title)
            return (
                <div style={{ marginTop: 30 }} className="ta_c">
                    无数据！
                    <p style={{ display: this.state.browserError, fontSize: 12, color: '#f60' }}>
                        想查看错题集请使用Chrome浏览器进行访问，谢谢～
                    </p>
                </div>
            )
        ...
        const row = (rowData, sectionID, rowID) => {
                    ...
                    <List renderHeader={() => question.title}>
                        {contentData.map((data, key) => (
                            <CheckboxItem
                                disabled={this.state.disabled}
                                key={data.value}
                                checked={this.state['checked' + data.value]}
                                onChange={() => this.onChange(data.value)}
                            >
                                {answers[key]}、{data.label}
                            </CheckboxItem>
            ...
            if (type > 2) {
                answerHtml = <List renderHeader={() => question.title}></List>
                color = '#000'
            }
            ...
            return (
                ...
                >
                    <div className="question_container">
                        ...
                        <Flex>
                            <Flex.Item>
                                <Button onClick={() => this.showQuestion()} className="btn ">
                                    查看答案
                                </Button>
                            </Flex.Item>
                            <Flex.Item>
                                <Button
                                    disabled={errorTotal === 1 ? true : false}
                        ...
                        <div style={{ display: 'none' }} id="showQuestion">
                            ...
                            {question.point && (
                                <List>
                                    <div style={{ lineHeight: '1.5', padding: '.3rem' }}>
                                        {question.point}
                                    </div>
                                </List>
                            )}
