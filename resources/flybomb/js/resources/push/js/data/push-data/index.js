...
const App = React.createClass({
	...
	tableData(searchParam) {//表格需要的数据
		...
		let url = restapi.listTaskStatics;
		if(type==='platform'){
			url=restapi.listTaskStaticsDetail;
		}
		...
		ajax.get(url, (result) => {
			...
			let appName =!appId ||  appId === '0' ? {//全部应用的时候表格里要显示应用包名
	...
	render() {
		return (
			<div>
				<Toolbar type={this.props.type} appName={this.state.appName} searchData={this.state.searchData} refresh={this.state.refresh} tableData={this.state.tableData} current={this.state.current} appId={this.state.appId} 	onSearch={this.tableData}  />
			</div>
		);
	}
});
